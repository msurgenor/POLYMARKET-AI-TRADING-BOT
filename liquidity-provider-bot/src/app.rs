...
impl App {
    pub async fn new(args: Vec<String>) -> Result<Self> {
        ...
        let collateral_addr_str = collateral_address.clone();
        order_book_manager.get_balances_with(move || {
            // Fetch balances from chain using contract calls
            let rt = tokio::runtime::Runtime::new().unwrap();
            let mut balances = HashMap::new();
            
            // Fetch collateral balance (ERC20)
            if !collateral_addr_str.is_empty() {
                if let Ok(collateral_addr) = collateral_addr_str.parse::<Address>() {
                    let balance = rt.block_on(
                        contracts_for_balances.token_balance_of(collateral_addr, address_for_balances, None)
                    );
                    balances.insert(COLLATERAL.to_string(), balance);
                }
            } else {
                balances.insert(COLLATERAL.to_string(), 0.0);
            }
            
            // Fetch TokenA balance (ERC1155)
            let token_a_id = market_for_balances.token_id(Token::A);
            // Note: Conditional token address would need to be fetched from API
            // For now, we'll try to fetch if we have the address
            balances.insert("TokenA".to_string(), 0.0);
            
            // Fetch TokenB balance (ERC1155)
            let token_b_id = market_for_balances.token_id(Token::B);
