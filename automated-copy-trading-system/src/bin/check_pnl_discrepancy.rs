...
async fn main() -> Result<()> {
    ...
    let closed_positions: Vec<_> = positions.iter().filter(|p| p.size == 0.0).collect();
    ...
    for (idx, pos) in open_positions.iter().enumerate() {
        ...
        println!("{}. {} - {}", idx + 1, pos.title.as_deref().unwrap_or("Unknown"), pos.outcome.as_deref().unwrap_or("N/A"));
    ...
    // Check through trade history
    println!("🔎 CHECK THROUGH TRADE HISTORY:\n");
    let activity_url = format!("https://data-api.polymarket.com/activity?user={}&type=TRADE", env.proxy_wallet);
    ...
    let mut market_trades: HashMap<String, (Vec<&Activity>, Vec<&Activity>)> = HashMap::new();

    for trade in &activities {
        let key = format!("{}:{}", trade.condition_id, trade.asset);
        let entry = market_trades.entry(key).or_insert_with(|| (Vec::new(), Vec::new()));
        if trade.side == "BUY" {
            entry.0.push(trade);
        } else {
            entry.1.push(trade);
    ...
    for (_key, (buys, sells)) in &market_trades {
        let total_bought: f64 = buys.iter().map(|t| t.usdc_size).sum();
        let total_sold: f64 = sells.iter().map(|t| t.usdc_size).sum();
        let pnl = total_sold - total_bought;

        if pnl.abs() > 0.01 {
            let market = buys.first().or_else(|| sells.first()).unwrap();
